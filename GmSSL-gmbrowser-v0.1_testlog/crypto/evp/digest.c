...
//该函数将参数d中的count字节数据进行信息摘要到ctx结构中去，该函数可以被调用多次，用以对更多的数据进行信息摘要。
...
/* The caller can assume that this removes any secret data from the context本函数将ctx结构中的摘要信息数据返回到参数md中 */
