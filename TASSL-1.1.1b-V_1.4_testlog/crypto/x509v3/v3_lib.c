...
extern const X509V3_EXT_METHOD GMold_v3_crld;
const X509V3_EXT_METHOD *GMold_X509V3_EXT_get(X509_EXTENSION *ext)
{
    int nid;
    if ((nid = GMold_OBJ_obj2nid(X509_EXTENSION_get_object(ext))) == NID_undef)
        return NULL;
    if(nid == 103)
        return &GMold_v3_crld;
    return X509V3_EXT_get_nid(nid);
}
...
void *GMold_X509V3_EXT_d2i(X509_EXTENSION *ext)
{
    ...
    if ((method = GMold_X509V3_EXT_get(ext)) == NULL)
        return NULL;
    extvalue = X509_EXTENSION_get_data(ext);
    p = ASN1_STRING_get0_data(extvalue);
    extlen = ASN1_STRING_length(extvalue);
    if (method->it)
        return GMold_ASN1_item_d2i(NULL, &p, extlen, ASN1_ITEM_ptr(method->it));
    return method->d2i(NULL, &p, extlen);
}
                     ...
                     int *idx)
{
    ...
    for (i = lastpos; i < sk_X509_EXTENSION_num(x); i++) {
        ex = sk_X509_EXTENSION_value(x, i);
        if (GMold_OBJ_obj2nid(X509_EXTENSION_get_object(ex)) == nid) {
            if (idx) {
                *idx = i;
                found_ex = ex;
                break;
    ...
    if (found_ex) {
        ...
        if (crit)
            *crit = X509_EXTENSION_get_critical(found_ex);
        return GMold_X509V3_EXT_d2i(found_ex);
    }
