...
{
    printf("dddddddddddd EC_KEY_set_group\n");
...
int GMold_EC_KEY_set_group(EC_KEY *key, const EC_GROUP *group)
{
    if (key->meth->set_group != NULL && key->meth->set_group(key, group) == 0)
        return 0;
    EC_GROUP_free(key->group);
    key->group = GMold_EC_GROUP_dup(group);
    return (key->group == NULL) ? 0 : 1;
}
                        ...
                        unsigned char **pbuf, BN_CTX *ctx)
{
    if (key == NULL || key->pub_key == NULL || key->group == NULL)
        return 0;
    return GMold_EC_POINT_point2buf(key->group, key->pub_key, form, pbuf, ctx);
}

int EC_KEY_oct2key(EC_KEY *key, const unsigned char *buf, size_t len,
                   BN_CTX *ctx)
{
    printf("dddddddddddd EC_KEY_oct2key\n");
                   ...
                   BN_CTX *ctx)
{
    ...
    if (key->pub_key == NULL)
        return 0;
    if (GMold_EC_POINT_oct2point(key->group, key->pub_key, buf, len, ctx) == 0)
        return 0;
    /*
     * Save the point conversion form.
     * For non-custom curves the first octet of the buffer (excluding
...
int GMold_ec_key_simple_oct2priv(EC_KEY *eckey, const unsigned char *buf, size_t len)
{
    ...
    eckey->priv_key = GMold_BN_bin2bn(buf, len, eckey->priv_key);
    if (eckey->priv_key == NULL) {
        return 0;
    }
    return 1;
