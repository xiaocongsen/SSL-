...
int GMold_ECPKParameters_print(BIO *bp, const EC_GROUP *x, int off)
{
    ...
    if (EC_GROUP_get_asn1_flag(x)) {
        ...
        if (nid == 0)
            goto err;
        if (BIO_printf(bp, "ASN1 OID: %s", GMold_OBJ_nid2sn(nid)) <= 0)
            goto err;
        if (BIO_printf(bp, "\n") <= 0)
            goto err;
        nname = GMold_EC_curve_nid2nist(nid);
        if (nname) {
            if (!BIO_indent(bp, off, 128))
                goto err;
            if (BIO_printf(bp, "NIST CURVE: %s\n", nname) <= 0)
        ...
        if (tmp_nid == GMold_NID_X9_62_characteristic_two_field)
            is_char_two = 1;
 ...
 err:
    if (!ret)
        printf("wocao xxxxxxxxxxxxxxxxx\n");
    BN_free(p);
    BN_free(a);
    BN_free(b);
    BN_free(gen);
