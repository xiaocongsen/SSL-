...
int ec_wNAF_mul(const EC_GROUP *group, EC_POINT *r, const BIGNUM *scalar,
                ...
                BN_CTX *ctx)
{
    printf("dddddddddddd ec_wNAF_mul\n");
                ...
                BN_CTX *ctx)
{
    ...
    if (scalar != NULL) {
        ...
        if (pre_comp && pre_comp->numblocks
             ...
             * yield (NB: maximum wNAF length is bit length plus one)
             */
            numblocks = (GMold_BN_num_bits(scalar) / blocksize) + 1;
    ...
    for (i = 0; i < num + num_scalar; i++) {
        ...
        bits = i < num ? GMold_BN_num_bits(scalars[i]) : GMold_BN_num_bits(scalar);
        wsize[i] = EC_window_bits_for_scalar_size(bits);
        num_val += (size_t)1 << (wsize[i] - 1);
        wNAF[i + 1] = NULL;     /* make sure we always have a pivot */
        wNAF[i] =
            GMold_bn_compute_wNAF((i < num ? scalars[i] : scalar), wsize[i],
                            &wNAF_len[i]);
        if (wNAF[i] == NULL)
            goto err;
        if (wNAF_len[i] > max_len)
    ...
    if (!GMold_EC_POINTs_make_affine(group, num_val, val, ctx))
        goto err;
