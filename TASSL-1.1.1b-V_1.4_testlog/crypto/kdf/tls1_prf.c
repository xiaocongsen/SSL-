...
static int GMold_tls1_prf_alg(const EVP_MD *md,
                        const unsigned char *sec, size_t slen,
                        const unsigned char *seed, size_t seed_len,
                        unsigned char *out, size_t olen);
...
static int pkey_tls1_prf_derive(EVP_PKEY_CTX *ctx, unsigned char *key,
                                size_t *keylen)
{
    printf("dddddd pkey_tls1_prf_derive\n");
    TLS1_PRF_PKEY_CTX *kctx = ctx->data;
    if (kctx->md == NULL) {
...
static int GMold_tls1_prf_P_hash(const EVP_MD *md,
                           const unsigned char *sec, size_t sec_len,
                           const unsigned char *seed, size_t seed_len,
                           unsigned char *out, size_t olen)
{
    ...
    if (ctx == NULL || ctx_tmp == NULL || ctx_init == NULL)
        goto err;
    EVP_MD_CTX_set_flags(ctx_init, EVP_MD_CTX_FLAG_NON_FIPS_ALLOW);
    mac_key = GMold_EVP_PKEY_new_mac_key(GMold_NID_hmac, NULL, sec, sec_len);
    if (mac_key == NULL)
        goto err;
    if (!GMold_EVP_DigestSignInit(ctx_init, NULL, md, NULL, mac_key))
        goto err;
    if (!EVP_MD_CTX_copy_ex(ctx, ctx_init))
        goto err;
    if (seed != NULL && !GMold_EVP_DigestUpdate(ctx, seed, seed_len))
        goto err;
    if (!GMold_EVP_DigestSignFinal(ctx, A1, &A1_len))
        goto err;
    ...
    for (;;) {
        ...
        if (!EVP_MD_CTX_copy_ex(ctx, ctx_init))
            goto err;
        if (!GMold_EVP_DigestUpdate(ctx, A1, A1_len))
            goto err;
        if (olen > (size_t)chunk && !EVP_MD_CTX_copy_ex(ctx_tmp, ctx))
            goto err;
        if (seed && !GMold_EVP_DigestUpdate(ctx, seed, seed_len))
            goto err;

        if (olen > (size_t)chunk) {
            size_t mac_len;
            if (!GMold_EVP_DigestSignFinal(ctx, out, &mac_len))
                goto err;
            out += mac_len;
            olen -= mac_len;
            /* calc the next A1 value */
            if (!GMold_EVP_DigestSignFinal(ctx_tmp, A1, &A1_len))
                goto err;
        } else {                /* last one */
...
static int GMold_tls1_prf_alg(const EVP_MD *md,
                        ...
                        unsigned char *out, size_t olen)
{
    if (EVP_MD_type(md) == GMold_NID_md5_sha1) {
        size_t i;
        unsigned char *tmp;
        if (!GMold_tls1_prf_P_hash(EVP_md5(), sec, slen/2 + (slen & 1),
                         seed, seed_len, out, olen))
            return 0;
        ...
        if (tmp == NULL)
            return 0;
        if (!GMold_tls1_prf_P_hash(EVP_sha1(), sec + slen/2, slen/2 + (slen & 1),
                         seed, seed_len, tmp, olen)) {
            OPENSSL_clear_free(tmp, olen);
            return 0;
        }
    ...
    if (!GMold_tls1_prf_P_hash(md, sec, slen, seed, seed_len, out, olen))
        return 0;

    return 1;
}

static int GMold_pkey_tls1_prf_init(EVP_PKEY_CTX *ctx)
{
    TLS1_PRF_PKEY_CTX *kctx;
...
static void GMold_pkey_tls1_prf_cleanup(EVP_PKEY_CTX *ctx)
{
    TLS1_PRF_PKEY_CTX *kctx = ctx->data;
    OPENSSL_clear_free(kctx->sec, kctx->seclen);
    OPENSSL_cleanse(kctx->seed, kctx->seedlen);
...
static int GMold_pkey_tls1_prf_derive(EVP_PKEY_CTX *ctx, unsigned char *key,
                                size_t *keylen)
{
    TLS1_PRF_PKEY_CTX *kctx = ctx->data;
    ...
    return GMold_tls1_prf_alg(kctx->md, kctx->sec, kctx->seclen,
                        kctx->seed, kctx->seedlen,
                        key, *keylen);
}


static int GMold_pkey_tls1_prf_ctrl(EVP_PKEY_CTX *ctx, int type, int p1, void *p2)
{
    TLS1_PRF_PKEY_CTX *kctx = ctx->data;
    switch (type) {
    case EVP_PKEY_CTRL_TLS_MD:
...
static int GMold_pkey_tls1_prf_ctrl_str(EVP_PKEY_CTX *ctx,
                                  const char *type, const char *value)
{
    if (value == NULL) {
        return 0;
...
const EVP_PKEY_METHOD GMold_tls1_prf_pkey_meth = {
    EVP_PKEY_TLS1_PRF,
    0,
    GMold_pkey_tls1_prf_init,
    0,
    GMold_pkey_tls1_prf_cleanup,
    ...
    0,
    GMold_pkey_tls1_prf_derive,
    GMold_pkey_tls1_prf_ctrl,
    GMold_pkey_tls1_prf_ctrl_str
};
