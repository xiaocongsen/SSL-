...
static int GMold_internal_find(OPENSSL_STACK *st, const void *data,
                         int ret_val_options)
{
    const void *r;
    int i;
    ...
    if (r == NULL)
        return (-1);
    return (int)((const char **)r - (const char **)st->data);
}
...
int GMold_OPENSSL_sk_find(OPENSSL_STACK *st, const void *data)
{
    return GMold_internal_find(st, data, OBJ_BSEARCH_FIRST_VALUE_ON_MATCH);
}
