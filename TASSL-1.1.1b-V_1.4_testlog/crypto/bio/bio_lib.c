...
int GMold_BIO_read(BIO *b, void *out, int outl)
{
    int i;
    long (*cb) (BIO *, int, const char *, int, long, long);

    if ((b == NULL) || (b->method == NULL) || (b->method->bread_old == NULL)) 
    {
        printf("GMold_BIO_read 111111111111111 i:%d\n",i);
        return (-2);
    }
    ...
    {
        printf("GMold_BIO_read 2222222222222222 i:%d\n",i);
        return (i);
    }
    ...
    if (!b->init) 
    {
        printf("GMold_BIO_read 33333333333333333 i:%d\n",i);
        return (-2);
    }

    i = b->method->bread_old(b, out, outl);
    if (i > 0)
        b->num_read += (uint64_t)i;

    if (cb != NULL)
        i = (int)cb(b, BIO_CB_READ | BIO_CB_RETURN, out, outl, 0L, (long)i);
//    printf("GMold_BIO_read 44444444444444444444444 i:%d\n",i);
    return (i);
}
...
int GMold_BIO_write(BIO *b, const void *in, int inl)
{
    int i = 0;
    long (*cb) (BIO *, int, const char *, int, long, long);
...
//    printf("GMold_BIO_write 44444444444444444444444 len:%d\n",inl);
    i = b->method->bwrite_old(b, in, inl);
    if (i > 0)
...
{
    int i;
    printf("ddddddddddd BIO_int_ctrl \n");
    i = iarg;
    return BIO_ctrl(b, cmd, larg, (char *)&i);
}

long GMold_BIO_int_ctrl(BIO *b, int cmd, long larg, int iarg)
{
    int i;

    i = iarg;
    return GMold_BIO_ctrl(b, cmd, larg, (char *)&i);
}
...
long GMold_BIO_ctrl(BIO *b, int cmd, long larg, void *parg)
{
    long ret;
    long (*cb) (BIO *, int, const char *, int, long, long);
...
BIO *GMold_BIO_push(BIO *b, BIO *bio)
{
    BIO *lb;
    ...
    if (bio != NULL)
        bio->prev_bio = lb;
    /* called to do internal processing */
    GMold_BIO_ctrl(b, BIO_CTRL_PUSH, 0, lb);
    return (b);
}
...
BIO *GMold_BIO_pop(BIO *b)
{
    ...
    GMold_BIO_ctrl(b, BIO_CTRL_POP, 0, b);
