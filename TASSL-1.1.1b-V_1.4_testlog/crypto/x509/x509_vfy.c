...
static int GMold_build_chain(X509_STORE_CTX *ctx);
static int verify_chain(X509_STORE_CTX *ctx);
static int GMold_verify_chain(X509_STORE_CTX *ctx);
static int dane_verify(X509_STORE_CTX *ctx);
static int null_callback(int ok, X509_STORE_CTX *e);
static int check_issued(X509_STORE_CTX *ctx, X509 *x, X509 *issuer);
static X509 *find_issuer(X509_STORE_CTX *ctx, STACK_OF(X509) *sk, X509 *x);
static X509 *GMold_find_issuer(X509_STORE_CTX *ctx, STACK_OF(X509) *sk, X509 *x);
static int check_chain_extensions(X509_STORE_CTX *ctx);
static int GMold_check_chain_extensions(X509_STORE_CTX *ctx);
static int check_name_constraints(X509_STORE_CTX *ctx);
static int check_id(X509_STORE_CTX *ctx);
static int check_trust(X509_STORE_CTX *ctx, int num_untrusted);
static int GMold_check_trust(X509_STORE_CTX *ctx, int num_untrusted);
...
static int GMold_check_key_level(X509_STORE_CTX *ctx, X509 *cert);
static int check_sig_level(X509_STORE_CTX *ctx, X509 *cert);
static int GMold_check_sig_level(X509_STORE_CTX *ctx, X509 *cert);
...
static int GMold_internal_verify(X509_STORE_CTX *ctx);
...
{
    printf("ddddddddddddddddd cert_self_signed\n");
    /*
...
static int GMold_cert_self_signed(X509 *x)
{
    /*
     * FIXME: x509v3_cache_extensions() needs to detect more failures and not
     * set EXFLAG_SET when that happens.  Especially, if the failures are
     * parse errors, rather than memory pressure!
     */
    GMold_X509_check_purpose(x, -1, 0);
    if (x->ex_flags & EXFLAG_SS)
        return 1;
    else
        return 0;
...
static int GMold_verify_cb_cert(X509_STORE_CTX *ctx, X509 *x, int depth, int err)
{
    ctx->error_depth = depth;
    ctx->current_cert = (x != NULL) ? x : sk_X509_value(ctx->chain, depth);
    if (err != X509_V_OK)
...
static int GMold_check_auth_level(X509_STORE_CTX *ctx)
{
    int i;
    int num = sk_X509_num(ctx->chain);
    ...
    for (i = 0; i < num; ++i) {
         ...
         * check the security of issuer keys.
         */
        if (i > 0 && !GMold_check_key_level(ctx, cert) &&
            verify_cb_cert(ctx, cert, i, X509_V_ERR_CA_KEY_TOO_SMALL) == 0)
            return 0;
        /*
         * We also check the signature algorithm security of all certificates
         * except those of the trust anchor at index num-1.
         */
        if (i < num - 1 && !GMold_check_sig_level(ctx, cert) &&
            verify_cb_cert(ctx, cert, i, X509_V_ERR_CA_MD_TOO_WEAK) == 0)
            return 0;
    }
    return 1;
...
static int verify_chain(X509_STORE_CTX *ctx)
{
    int err;
    int ok;
    printf("ddddddddddddddddd verify_chain\n");
...
static int GMold_verify_chain(X509_STORE_CTX *ctx)
{
     ...
     * instantiate chain public key parameters.
     */
    if ((ok = GMold_build_chain(ctx)) == 0 ||
        (ok = GMold_check_chain_extensions(ctx)) == 0 ||
        (ok = GMold_check_auth_level(ctx)) == 0 ||
        (ok = check_name_constraints(ctx)) == 0 ||
        (ok = check_id(ctx)) == 0 || 1)
        GMold_X509_get_pubkey_parameters(NULL, ctx->chain);
    if (ok == 0 || (ok = ctx->check_revocation(ctx)) == 0)
        return ok;
    ...
    /* Verify chain signatures and expiration times */
    ok = (ctx->verify != NULL) ? ctx->verify(ctx) : GMold_internal_verify(ctx);
    if (!ok)
        return ok;
...
{
    printf("ddddddddddddddddd X509_verify_cert\n");
...
int GMold_X509_verify_cert(X509_STORE_CTX *ctx)
{
    SSL_DANE *dane = ctx->dane;
    int ret;
    ...
    /* If the peer's public key is too weak, we can stop early. */
    if (!GMold_check_key_level(ctx, ctx->cert) &&
        !verify_cb_cert(ctx, ctx->cert, 0, X509_V_ERR_EE_KEY_TOO_SMALL))
        return 0;
...
static int check_issued(X509_STORE_CTX *ctx, X509 *x, X509 *issuer)
{
    printf("ddddddddddd check_issued \n");
    int ret;
    if (x == issuer)
...
static int GMold_check_issued(X509_STORE_CTX *ctx, X509 *x, X509 *issuer)
{
    int ret;
    if (x == issuer)
        return GMold_cert_self_signed(x);
    ret = GMold_X509_check_issued(issuer, x);
    if (ret == X509_V_OK) {
        int i;
        X509 *ch;
        /* Special case: single self signed certificate */
        if (GMold_cert_self_signed(x) && sk_X509_num(ctx->chain) == 1)
            return 1;
        for (i = 0; i < sk_X509_num(ctx->chain); i++) {
            ch = sk_X509_value(ctx->chain, i);
            if (ch == issuer || !GMold_X509_cmp(ch, issuer)) {
                ret = X509_V_ERR_PATH_LOOP;
                break;
            }
        }
...
static int GMold_check_purpose(X509_STORE_CTX *ctx, X509 *x, int purpose, int depth,
                         int must_be_ca)
{
    int tr_ok = X509_TRUST_UNTRUSTED;
     ...
     */
    if (depth >= ctx->num_untrusted && purpose == ctx->param->purpose)
        tr_ok = GMold_X509_check_trust(x, ctx->param->trust, X509_TRUST_NO_SS_COMPAT);
    ...
    case X509_TRUST_REJECTED:
        break;
    default:
        switch (GMold_X509_check_purpose(x, purpose, must_be_ca > 0)) {
        case 1:
            return 1;
        case 0:
            break;
...
static int GMold_check_chain_extensions(X509_STORE_CTX *ctx)
{
    ...
    for (i = 0; i < num; i++) {
        ...
        if (!(ctx->param->flags & X509_V_FLAG_IGNORE_CRITICAL)
            && (x->ex_flags & EXFLAG_CRITICAL)) {
            if (!GMold_verify_cb_cert(ctx, x, i,
                                X509_V_ERR_UNHANDLED_CRITICAL_EXTENSION))
                return 0;
        }
        if (!allow_proxy_certs && (x->ex_flags & EXFLAG_PROXY)) {
        ...
        if (ret == 0 && !verify_cb_cert(ctx, x, i, X509_V_OK))
            return 0;
        /* check_purpose() makes the callback as needed */
        if (purpose > 0 && !GMold_check_purpose(ctx, x, purpose, i, must_be_ca))
            return 0;
...
static int check_trust(X509_STORE_CTX *ctx, int num_untrusted)
{
    printf("ddddddddddd check_trust\n");
    int i;
    X509 *x = NULL;
    X509 *mx;
    SSL_DANE *dane = ctx->dane;
...
static int GMold_check_trust(X509_STORE_CTX *ctx, int num_untrusted)
{
    ...
    for (i = num_untrusted; i < num; i++) {
        x = sk_X509_value(ctx->chain, i);
        trust = GMold_X509_check_trust(x, ctx->param->trust, 0);
        /* If explicitly trusted return trusted */
        if (trust == X509_TRUST_TRUSTED)
            goto trusted;
        if (trust == X509_TRUST_REJECTED)
    ...
    if (num_untrusted == num && ctx->param->flags & X509_V_FLAG_PARTIAL_CHAIN) {
         ...
         * we'll accept X509_TRUST_UNTRUSTED when not self-signed.
         */
        trust = GMold_X509_check_trust(mx, ctx->param->trust, 0);
        if (trust == X509_TRUST_REJECTED) {
            X509_free(mx);
            goto rejected;
        }
...
static int GMold_internal_verify(X509_STORE_CTX *ctx)
{
    ...
    while (n >= 0) {
         ...
         */
        if (xs != xi || (ctx->param->flags & X509_V_FLAG_CHECK_SS_SIGNATURE)) {
            if ((pkey = GMold_X509_get0_pubkey(xi)) == NULL) {
                if (!verify_cb_cert(ctx, xi, xi != xs ? n+1 : n,
                        X509_V_ERR_UNABLE_TO_DECODE_ISSUER_PUBLIC_KEY))
                    return 0;
            } else if (GMold_X509_verify(xs, pkey) <= 0) {
                if (!verify_cb_cert(ctx, xs, n,
                                    X509_V_ERR_CERT_SIGNATURE_FAILURE))
                    return 0;
            }
...
int GMold_X509_get_pubkey_parameters(EVP_PKEY *pkey, STACK_OF(X509) *chain)
{
    EVP_PKEY *ktmp = NULL, *ktmp2;
    int i, j;
    ...
    for (i = 0; i < sk_X509_num(chain); i++) {
        ktmp = GMold_X509_get0_pubkey(sk_X509_value(chain, i));
        if (ktmp == NULL) {
            return 0;
        }
        if (!EVP_PKEY_missing_parameters(ktmp))
    ...
    /* first, populate the other certs */
    for (j = i - 1; j >= 0; j--) {
        ktmp2 = GMold_X509_get0_pubkey(sk_X509_value(chain, j));
        EVP_PKEY_copy_parameters(ktmp2, ktmp);
    }
...
int X509_STORE_CTX_init(X509_STORE_CTX *ctx, X509_STORE *store, X509 *x509,
                        STACK_OF(X509) *chain)
{
    printf("ddddddddddd X509_STORE_CTX_init \n");
    int ret = 1;
...
int GMold_X509_STORE_CTX_init(X509_STORE_CTX *ctx, X509_STORE *store, X509 *x509,
                        STACK_OF(X509) *chain)
{
    int ret = 1;
    ...
    if (store && store->check_issued)
        ...
        ctx->check_issued = GMold_check_issued;

    if (store && store->get_issuer)
        ctx->get_issuer = store->get_issuer;
    else
        ctx->get_issuer = GMold_X509_STORE_CTX_get1_issuer;
    ...
    if (store && store->verify)
        ...
        ctx->verify = GMold_internal_verify;
...
{
    printf("ddddddddddddddddd get_issuer\n");
...
/* Get issuer, without duplicate suppression */
static int GMold_get_issuer(X509 **issuer, X509_STORE_CTX *ctx, X509 *cert)
{
    STACK_OF(X509) *saved_chain = ctx->chain;
    int ok;
...
static int build_chain(X509_STORE_CTX *ctx)
{
    printf("ddddddddddddddddd build_chain\n");
    SSL_DANE *dane = ctx->dane;
    int num = sk_X509_num(ctx->chain);
    X509 *cert = sk_X509_value(ctx->chain, num - 1);
...
static int GMold_build_chain(X509_STORE_CTX *ctx)
{
    ...
    int num = sk_X509_num(ctx->chain);
    X509 *cert = sk_X509_value(ctx->chain, num - 1);
    int ss = GMold_cert_self_signed(cert);
    STACK_OF(X509) *sktmp = NULL;
    unsigned int search;
    int may_trusted = 0;
    int may_alternate = 0;
...
#define S_DOALTERNATE      (1 << 2)     /* Retry with pruned alternate chain */
    ...
    while (search != 0) {
        ...
        if ((search & S_DOTRUSTED) != 0) {
            ...
            ok = (depth < num) ? 0 : GMold_get_issuer(&xtmp, ctx, x);
            ...
            if (ok > 0) {
                 ...
                 * At this point ctx->num_trusted and num must reflect the
                 * correct number of untrusted certificates, since the DANE
                 * logic in GMold_check_trust() depends on distinguishing CAs from
                 * "the wire" from CAs from the trust store.  In particular, the
                 * certificate at depth "num" should be the new trusted
                 * certificate with ctx->num_untrusted <= num.
                 */
                if (ok) {
                    ...
                    search &= ~S_DOUNTRUSTED;
                    switch (trust = GMold_check_trust(ctx, num)) {
                    case X509_TRUST_TRUSTED:
                    case X509_TRUST_REJECTED:
                        search = 0;
                        continue;
        ...
        if ((search & S_DOUNTRUSTED) != 0) {
             ...
             * and start looking only in the trust store if enabled.
             */
            xtmp = (ss || depth < num) ? NULL : GMold_find_issuer(ctx, sktmp, x);
            if (xtmp == NULL) {
                search &= ~S_DOUNTRUSTED;
                if (may_trusted)
                    search |= S_DOTRUSTED;
            ...
            X509_up_ref(x = xtmp);
            ++ctx->num_untrusted;
            ss = GMold_cert_self_signed(xtmp);
    ...
    if (num <= depth) {
        if (trust == X509_TRUST_UNTRUSTED && DANETLS_HAS_DANE_TA(dane))
            trust = check_dane_pkeys(ctx);
        if (trust == X509_TRUST_UNTRUSTED && num == ctx->num_untrusted)
            trust = GMold_check_trust(ctx, num);
    }
...
static int check_key_level(X509_STORE_CTX *ctx, X509 *cert)
{
    printf("ddddddddddd check_key_level \n");
...
static int GMold_check_key_level(X509_STORE_CTX *ctx, X509 *cert)
{
    EVP_PKEY *pkey = GMold_X509_get0_pubkey(cert);
    int level = ctx->param->auth_level;
    ...
    return GMold_EVP_PKEY_security_bits(pkey) >= minbits_table[level - 1];
}
...
static int GMold_check_sig_level(X509_STORE_CTX *ctx, X509 *cert)
{
    int nid = GMold_X509_get_signature_nid(cert);
    int mdnid = GMold_NID_undef;
    int secbits = -1;
    int level = ctx->param->auth_level;
    ...
    if (nid && GMold_OBJ_find_sigid_algs(nid, &mdnid, NULL)) {
        const EVP_MD *md;

        /* Assume 4 bits of collision resistance for each hash octet */
        if(mdnid == GMold_NID_sm3)
        {
            md = GMold_EVP_sm3();
            secbits = EVP_MD_size(md) * 4;
        }
        else if (mdnid != GMold_NID_undef && (md = EVP_get_digestbyname(GMold_OBJ_nid2sn(mdnid))) != NULL)
        {
            secbits = EVP_MD_size(md) * 4;
        }
    }
