...
static int GMold_x509_name_ex_d2i(ASN1_VALUE **val,
                            const unsigned char **in, long len,
...
static int GMold_x509_name_canon(X509_NAME *a);
static int asn1_string_canon(ASN1_STRING *out, const ASN1_STRING *in);
static int i2d_name_canon(STACK_OF(STACK_OF_X509_NAME_ENTRY) * intname,
                          unsigned char **in);
static int GMold_i2d_name_canon(STACK_OF(STACK_OF_X509_NAME_ENTRY) * _intname,
                          unsigned char **in);
static int x509_name_ex_print(BIO *out, ASN1_VALUE **pval,
                              int indent,
                              const char *fname, const ASN1_PCTX *pctx);
extern int GMold_ASN1_item_ex_d2i(ASN1_VALUE **pval, const unsigned char **in, long len,
                     const ASN1_ITEM *it,
                     int tag, int aclass, char opt, ASN1_TLC *ctx);
...
static const ASN1_EXTERN_FUNCS GMold_x509_name_ff = {
    NULL,
    x509_name_ex_new,
    x509_name_ex_free,
    0,                          /* Default clear behaviour is OK */
    GMold_x509_name_ex_d2i,
    x509_name_ex_i2d,
    x509_name_ex_print
};

const ASN1_ITEM GMold_X509_NAME_it = { 
    ASN1_ITYPE_EXTERN, V_ASN1_SEQUENCE, NULL, 0, &GMold_x509_name_ff, 0, "X509_NAME" 
    };
...
static int x509_name_ex_d2i(ASN1_VALUE **val,
                            ...
                            char opt, ASN1_TLC *ctx)
{
    printf("ddddddddddd x509_name_ex_d2i \n");
...
static void GMold_name_entry_stack_free(STACK_OF(X509_NAME_ENTRY) *ents)
{
                            ...
                            char opt, ASN1_TLC *ctx)
{
    ...
    /* Get internal representation of Name */
    ret = GMold_ASN1_item_ex_d2i(&intname.a,
                           &p, len, ASN1_ITEM_rptr(X509_NAME_INTERNAL),
                           tag, aclass, opt, ctx);
    ...
    intname.s = NULL;
    ret = GMold_x509_name_canon(nm.x);
    if (!ret)
        goto err;
    nm.x->modified = 0;
    *val = nm.a;
 ...
 err:
    ...
    if (intname.s != NULL) {
        ...
        sk_STACK_OF_X509_NAME_ENTRY_pop_free(intname.s, GMold_name_entry_stack_free);
    }
    X509_NAME_free(nm.x);
    return 0;
}
...
static int x509_name_ex_i2d(ASN1_VALUE **val, unsigned char **out,
                            const ASN1_ITEM *it, int tag, int aclass)
{
    printf("ddddddddddd x509_name_ex_i2d \n");
    int ret;
...
static int GMold_x509_name_canon(X509_NAME *a)
{
    ...
    len = GMold_i2d_name_canon(intname, NULL);
    if (len < 0)
        goto err;
    a->canon_enclen = len;
    ...
    GMold_i2d_name_canon(intname, &p);
...
static int i2d_name_canon(STACK_OF(STACK_OF_X509_NAME_ENTRY) * _intname,
                          unsigned char **in)
{
    printf("ddddddddddddd i2d_name_canon\n");
    int i, len, ltmp;
    ASN1_VALUE *v;
                          ...
                          unsigned char **in)
{
    ...
    for (i = 0; i < sk_ASN1_VALUE_num(intname); i++) {
        v = sk_ASN1_VALUE_value(intname, i);
        ltmp = GMold_ASN1_item_ex_i2d(&v, in,
                                ASN1_ITEM_rptr(X509_NAME_ENTRIES), -1, -1);
        if (ltmp < 0)
            return ltmp;
        len += ltmp;
...
int GMold_X509_NAME_print(BIO *bp, const X509_NAME *name, int obase)
{
    ...
    b = GMold_X509_NAME_oneline(name, NULL, 0);
    if (!b)
        return 0;
    if (!*b) {
        OPENSSL_free(b);
