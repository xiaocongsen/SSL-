...
int GMold_X509_cmp(const X509 *a, const X509 *b)
{
    int rv;
    /* ensure hash is valid */
    GMold_X509_check_purpose((X509 *)a, -1, 0);
    GMold_X509_check_purpose((X509 *)b, -1, 0);
...
{
    printf("ddddddddddd X509_get0_pubkey\n");
    if (x == NULL)
        return NULL;
...
EVP_PKEY *GMold_X509_get0_pubkey(const X509 *x)
{
    if (x == NULL)
        return NULL;
    return GMold_X509_PUBKEY_get0(x->cert_info.key);
}
