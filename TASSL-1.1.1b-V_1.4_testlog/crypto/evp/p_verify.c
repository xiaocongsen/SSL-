...
int GMold_EVP_VerifyFinal(EVP_MD_CTX *ctx, const unsigned char *sigbuf,
                    unsigned int siglen, EVP_PKEY *pkey)
{
    unsigned char m[EVP_MAX_MD_SIZE];
    unsigned int m_len = 0;
    ...
    if (EVP_PKEY_id(pkey) == GMold_EVP_PKEY_EC && EC_GROUP_get_curve_name(
        EC_KEY_get0_group(EVP_PKEY_get0_EC_KEY(pkey))) == GMold_NID_sm2p256v1) {
            ...
            if (GMold_EVP_PKEY_CTX_ctrl(pkctx, GMold_EVP_PKEY_EC, EVP_PKEY_OP_SIGN|EVP_PKEY_OP_SIGNCTX| EVP_PKEY_OP_VERIFY|EVP_PKEY_OP_VERIFYCTX| EVP_PKEY_OP_ENCRYPT|EVP_PKEY_OP_DECRYPT| EVP_PKEY_OP_DERIVE, EVP_PKEY_CTRL_EC_SCHEME, GMold_NID_sm_scheme, NULL) <= 0)
            {
                goto err;
            }
    }
