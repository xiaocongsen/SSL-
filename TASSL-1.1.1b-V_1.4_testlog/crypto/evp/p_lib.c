...
{
    printf("dddddd EVP_PKEY_security_bits\n");
    if (pkey == NULL)
        return 0;
...
static int GMold_pkey_set_type(EVP_PKEY *pkey, int type, const char *str, int len)
{
    const EVP_PKEY_ASN1_METHOD *ameth;
    ENGINE *e = NULL;
    if (pkey) {
    ...
    if(type == GMold_NID_hmac)
        ameth = GMold_hmac_asn1_method();
    else if(type == GMold_NID_X9_62_id_ecPublicKey)
        ameth = GMold_eckey_asn1_method();
    else if (str)
...
{
    printf("dddddd EVP_PKEY_set_type\n");
    return pkey_set_type(pkey, NULL, type, NULL, -1);
}

int GMold_EVP_PKEY_set_type(EVP_PKEY *pkey, int type)
{
    return GMold_pkey_set_type(pkey, type, NULL, -1);
}
...
{
    if (pkey == NULL || !GMold_EVP_PKEY_set_type(pkey, type))
        return 0;
    pkey->pkey.ptr = key;
    return (key != NULL);
}
