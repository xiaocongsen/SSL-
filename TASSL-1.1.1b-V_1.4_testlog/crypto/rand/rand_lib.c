...
#  ifdef DEVRANDOM
    ...
    for (i = 0; (i < OSSL_NELEM(randomfiles)) && (n < GMold_ENTROPY_NEEDED); i++) {
...
#   endif
                ...
                if (try_read) {
                    r = read(fd, (unsigned char *)tmpbuf + n,
                             GMold_ENTROPY_NEEDED - n);
//                    memset(tmpbuf+n , 2,GMold_ENTROPY_NEEDED - n);        //TODO xcst
                    if (r > 0)
                        n += r;
                } else
                    r = -1;
...
#  if !defined(OPENSSL_NO_EGD) && defined(DEVRANDOM_EGD)
    ...
    for (egdsocket = egdsockets; *egdsocket && n < GMold_ENTROPY_NEEDED;
         egdsocket++) {
        int r;
...
#  if defined(DEVRANDOM) || (!defined(OPENSSL_NO_EGD) && defined(DEVRANDOM_EGD))
    if (n > 0) {
        GMold_RAND_add(tmpbuf, sizeof tmpbuf, (double)n);
...
#  endif
    ...
    l = curr_pid;
    GMold_RAND_add(&l, sizeof(l), 0.0);
    l = getuid(); //TODO xcst
    GMold_RAND_add(&l, sizeof(l), 0.0);

    l = time(NULL);  //TODO xcst
    GMold_RAND_add(&l, sizeof(l), 0.0);
...
{
//    printf("ddddddddddddd RAND_get_rand_method\n");
...
const RAND_METHOD *GMold_RAND_get_rand_method(void)
{
    return GMold_RAND_OpenSSL();
}
...
{
    printf("ddddddddddddd RAND_seed\n");
    const RAND_METHOD *meth = RAND_get_rand_method();
...
{
    const RAND_METHOD *meth = GMold_RAND_get_rand_method();
    if (meth && meth->seed)
        meth->seed(buf, num);
}
...
void GMold_RAND_add(const void *buf, int num, double entropy)
{
    const RAND_METHOD *meth = GMold_RAND_get_rand_method();
    if (meth && meth->add)
...
int GMold_RAND_priv_bytes(unsigned char *buf, int num)
{
    const RAND_METHOD *meth = GMold_RAND_get_rand_method();
    RAND_DRBG *drbg;
    int ret;

    if (meth != GMold_RAND_OpenSSL())
        return GMold_RAND_bytes(buf, num);
...
{
//    printf("ddddddddddddd RAND_bytes\n");
    const RAND_METHOD *meth = RAND_get_rand_method();
...
int GMold_RAND_bytes(unsigned char *buf, int num)
{
    const RAND_METHOD *meth = GMold_RAND_get_rand_method();
    if (meth && meth->bytes)
        return meth->bytes(buf, num);
