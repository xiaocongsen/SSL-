...
{
    printf("ddddddddddd ssl3_get_record \n");
    int enc_err, rret;
    int i;
    size_t more, n;
...
int GMold_ssl3_get_record(SSL *s)
{
    ...
    do {
        ...
        if ((RECORD_LAYER_get_rstate(&s->rlayer) != SSL_ST_READ_BODY) ||
            (RECORD_LAYER_get_packet_length(&s->rlayer)
             < SSL3_RT_HEADER_LENGTH)) {
            n = GMold_ssl3_read_n(s, SSL3_RT_HEADER_LENGTH,
                            SSL3_BUFFER_get_len(rbuf), 0,
                            num_recs == 0 ? 1 : 0);
            if (n <= 0)
            {
                printf("GMold_ssl3 get_record 1111111111111111111111111111\n");
            ...
            if (s->server && RECORD_LAYER_is_first_record(&s->rlayer)
                ...
                if (rr[num_recs].length > SSL3_BUFFER_get_len(rbuf)
                    ...
                    printf("GMold_ssl3 get_record 222222222222\n");
                    goto f_err;
                }

                if (rr[num_recs].length < MIN_SSL2_RECORD_LEN) {
                    al = SSL_AD_HANDSHAKE_FAILURE;
                    printf("GMold_ssl3 get_record 3333333333333\n");
                    goto f_err;
                }
            } else {
                /* SSLv3+ style record */
                ...
                if (!s->first_packet && version != s->version) {
                    if ((s->version & 0xFF00) == (version & 0xFF00)
                        ...
                        if (rr->type == SSL3_RT_ALERT) {
                             ...
                             * end.
                             */
                            printf("GMold_ssl3 get_record 4444444444444444444\n");
                            goto err;
                        }
                        /*
                         * Send back error using their minor version number :-)
                    ...
                    printf("GMold_ssl3 get_record 55555555555555555555\n");
                    goto f_err;
                }
                ...
                else if ((version >> 8) != SSL3_VERSION_MAJOR) {
                    if (RECORD_LAYER_is_first_record(&s->rlayer)) {
                        /* Go back to start of packet, look at the five bytes
                         * that we have. */
                        p = RECORD_LAYER_get_packet(&s->rlayer);
                        if (strncmp((char *)p, "GET ", 4) == 0 ||
                            ...
                            strncmp((char *)p, "HEAD ", 5) == 0 ||
                            strncmp((char *)p, "PUT ", 4) == 0) {
                                printf("GMold_ssl3 get_record 66666666666666\n");
                            goto err;
                        } else if (strncmp((char *)p, "CONNE", 5) == 0) {
                            printf("GMold_ssl3 get_record 77777777777777777777\n");
                            goto err;
                        }

                        /* Doesn't look like TLS - don't send an alert */
                        printf("GMold_ssl3 get_record 888888888888888888\n");
                        goto err;
                    } else {
                        al = SSL_AD_PROTOCOL_VERSION;
                        printf("GMold_ssl3 get_record 999999999999999999\n");
                        goto f_err;
                    }
                }
                ...
                if (rr[num_recs].length >
                    SSL3_BUFFER_get_len(rbuf) - SSL3_RT_HEADER_LENGTH) {
                    al = SSL_AD_RECORD_OVERFLOW;
                    printf("GMold_ssl3 get_record aaaaaaaaaaaaaaaaaaaaa\n");
                    goto f_err;
                }
            }
        ...
        if (i > 0) {
            ...
            n = GMold_ssl3_read_n(s, i, i, 1, 0);
            if (n <= 0)
            {
                printf("GMold_ssl3 get_record bbbbbbbbbbbbbbbbbbbbbbbbbbb\n");
        ...
        if (rr[num_recs].length > SSL3_RT_MAX_ENCRYPTED_LENGTH) {
            al = SSL_AD_RECORD_OVERFLOW;
            printf("GMold_ssl3 get_record ccccccccccccccccccccccccc\n");
            goto f_err;
        }
     ...
     * the details below are public so no timing details can leak.
     */
    if ((s->s3->flags & TLS1_FLAGS_ENCRYPT_THEN_MAC) && s->read_hash) {
        unsigned char *mac;
        mac_size = EVP_MD_CTX_size(s->read_hash);
        ...
        for (j = 0; j < num_recs; j++) {
            if (rr[j].length < mac_size) {
                ...
                printf("GMold_ssl3 get_record dddddddddddddddddddddddddd\n");
                goto f_err;
            }
            ...
            if (i < 0 || CRYPTO_memcmp(md, mac, (size_t)mac_size) != 0) {
                ...
                printf("GMold_ssl3 get_record eeeeeeeeeeeeeeeeeeeeeeee\n");
                goto f_err;
            }
        }
    }
    ...
    if (enc_err == 0) {
        ...
        printf("GMold_ssl3 get_record fffffffffffffffffffffffff\n");
        goto f_err;
    }
    ...
    if ((sess != NULL) &&
        (s->enc_read_ctx != NULL) &&
        (EVP_MD_CTX_md(s->read_hash) != NULL) && !(s->s3->flags & TLS1_FLAGS_ENCRYPT_THEN_MAC)) {
        /* s->read_hash != NULL => mac_size != -1 */
        unsigned char *mac = NULL;
        unsigned char mac_tmp[EVP_MAX_MD_SIZE];
        ...
        for (j = 0; j < num_recs; j++) {
            ...
            if (rr[j].orig_len < mac_size ||
                ...
                al = SSL_AD_DECODE_ERROR;
                printf("GMold_ssl3 get_record ggggggggggggggggggggggggg\n");
                goto f_err;
            }
    ...
    if (enc_err < 0) {
         ...
         */
        al = SSL_AD_BAD_RECORD_MAC;
        printf("GMold_ssl3 get_record hhhhhhhhhhhhhhhhhhhhhh\n");
        goto f_err;
    }
    ...
    for (j = 0; j < num_recs; j++) {
        ...
        if (s->expand != NULL) {
            if (rr[j].length > SSL3_RT_MAX_COMPRESSED_LENGTH) {
                al = SSL_AD_RECORD_OVERFLOW;
                printf("GMold_ssl3 get_record iiiiiiiiiiiiiiiiiiiiiiiii\n");
                goto f_err;
            }
            if (!ssl3_do_uncompress(s, &rr[j])) {
                al = SSL_AD_DECOMPRESSION_FAILURE;
                printf("GMold_ssl3 get_record jjjjjjjjjjjjjjjjjjjjjjjjjjj\n");
                goto f_err;
            }
        }
        ...
        if (rr[j].length > SSL3_RT_MAX_PLAIN_LENGTH) {
            al = SSL_AD_RECORD_OVERFLOW;
            printf("GMold_ssl3 get_record kkkkkkkkkkkkkkkkkkkkkkkkk\n");
            goto f_err;
        }
        ...
        if (rr[j].length == 0) {
            ...
            if (RECORD_LAYER_get_empty_record_count(&s->rlayer)
                ...
                printf("GMold_ssl3 get_record lllllllllllllllllllllll\n");
                goto f_err;
            }
        } else {
            RECORD_LAYER_reset_empty_record_count(&s->rlayer);
...
{
    printf("ddddddddddddddddd tls1_enc\n");
    EVP_CIPHER_CTX *ds;
...
int GMold_tls1_enc(SSL *s, SSL3_RECORD *recs, size_t n_recs, int send)
{
    EVP_CIPHER_CTX *ds;
    size_t reclen[SSL_MAX_PIPELINES];
    unsigned char buf[SSL_MAX_PIPELINES][EVP_AEAD_TLS1_AAD_LEN];
            ...
            if (ivlen > 1) {
                for (ctr = 0; ctr < n_recs; ctr++) {
                    if (recs[ctr].data != recs[ctr].input) {
                         ...
                         */
                        return -1;
                    } else if (GMold_RAND_bytes(recs[ctr].input, ivlen) <= 0) {
                        return -1;
                    }
                }
            }
    ...
    if ((s->session == NULL) || (ds == NULL) || (enc == NULL)) {
        ...
        ret = 1;
        if (!(s->s3->flags & TLS1_FLAGS_ENCRYPT_THEN_MAC) && EVP_MD_CTX_md(s->read_hash) != NULL)
            mac_size = EVP_MD_CTX_size(s->read_hash);
        if ((bs != 1) && !send) {
            int tmpret;
            for (ctr = 0; ctr < n_recs; ctr++) {
...
int GMold_tls_fips_digest_extra(const EVP_CIPHER_CTX *cipher_ctx,
                          ...
                          size_t data_len, size_t orig_len)
{
     ...
     */
    return GMold_EVP_DigestUpdate(mac_ctx, data,
...
{
    printf("ddddddddddddddddd tls1_mac\n");
    unsigned char *seq;
    EVP_MD_CTX *hash;
    size_t md_size;
    int i;
...
int GMold_tls1_mac(SSL *ssl, SSL3_RECORD *rec, unsigned char *md, int send)
{
    unsigned char *seq;
    EVP_MD_CTX *hash;
    size_t md_size;
    ...
    if (!send && !(ssl->s3->flags & TLS1_FLAGS_ENCRYPT_THEN_MAC) &&
        EVP_CIPHER_CTX_mode(ssl->enc_read_ctx) == EVP_CIPH_CBC_MODE &&
        ssl3_cbc_record_digest_supported(mac_ctx)) {
        /*
         * This is a CBC-encrypted record. We must avoid leaking any
        ...
        if (GMold_EVP_DigestUpdate(mac_ctx, header, sizeof(header)) <= 0
            || GMold_EVP_DigestUpdate(mac_ctx, rec->input, rec->length) <= 0
            || EVP_DigestSignFinal(mac_ctx, md, &md_size) <= 0) {
            EVP_MD_CTX_free(hmac);
            return -1;
        }
        if (!send && !(ssl->s3->flags & TLS1_FLAGS_ENCRYPT_THEN_MAC) && FIPS_mode())
            if (!GMold_tls_fips_digest_extra(ssl->enc_read_ctx,
                                       mac_ctx, rec->input,
