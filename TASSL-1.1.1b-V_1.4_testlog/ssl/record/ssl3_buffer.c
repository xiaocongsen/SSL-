...
{
    printf("ddddddddddddddddd ssl3_setup_read_buffer\n");
    unsigned char *p;
    ...
    if (b->buf == NULL) {
        ...
        if ((p = OPENSSL_malloc(len)) == NULL) {
            ...
            printf("xxxxxxxx ssl3_setup read_buffer OPENSSL_malloc error\n");
            return 0;
        }
        b->buf = p;
        b->len = len;
...
int GMold_ssl3_setup_read_buffer(SSL *s)
{
    unsigned char *p;
    size_t len, align = 0, headerlen;
    SSL3_BUFFER *b;
    ...
    if (b->buf == NULL) {
        len = SSL3_RT_MAX_PLAIN_LENGTH
            + SSL3_RT_MAX_ENCRYPTED_OVERHEAD + headerlen + align;
        if (GMold_ssl_allow_compression(s))
            len += SSL3_RT_MAX_COMPRESSED_OVERHEAD;
        if (b->default_len > len)
            len = b->default_len;
        if ((p = OPENSSL_malloc(len)) == NULL)
...
{
    printf("ddddddddddddddddd ssl3_setup_write_buffer\n");
    unsigned char *p;
...
int GMold_ssl3_setup_write_buffer(SSL *s, unsigned int numwpipes, size_t len)
{
    unsigned char *p;
    size_t align = 0, headerlen;
    SSL3_BUFFER *wb;
...
int GMold_ssl3_setup_buffers(SSL *s)
{
    if (!GMold_ssl3_setup_read_buffer(s))
        return 0;
    if (!GMold_ssl3_setup_write_buffer(s, 1, 0))
        return 0;
    return 1;
}
