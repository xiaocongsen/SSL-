                ...
                size_t *readbytes)
{
    ...
    unsigned char *pkt;
    SSL3_BUFFER *rb;
    printf("ddddddddddd ssl3_read_n \n");
    if (n == 0)
        return 0;
 ...
 * Return values are as per SSL_read()
 */
int GMold_ssl3_read_n(SSL *s, int n, int max, int extend, int clearold)
{
    /*
     * If extend == 0, obtain new n-byte packet; if extend == 1, increase
     * packet by another n bytes. The packet will be in the sub-array of
    ...
    if (n <= 0)
    {
        printf("GMold_ssl3 read_n 1111111111111111111111111111\n");
        return n;
    }
    ...
    if (rb->buf == NULL)
        if (!ssl3_setup_read_buffer(s))
        {
            printf("GMold_ssl3 read_n 22222222222222222222222222222\n");
            return -1;
        }
    ...
    if (SSL_IS_DTLS(s)) {
        if (left == 0 && extend)
        {
            printf("GMold_ssl3 read_n 33333333333333333333333333\n");
            return 0;
        }
    ...
    if (left >= n) {
        ...
        rb->left = left - n;
        rb->offset += n;
        printf("GMold_ssl3 read_n 4444444444444444444444444444 n:%d\n",n);
        return (n);
    }
    ...
    if (n > (int)(rb->len - rb->offset)) { /* does not happen */
        printf("GMold_ssl3 read_n 55555555555555555555\n");
        return -1;
    }
    ...
    while (left < n) {
        ...
        if (s->rbio != NULL) {
            s->rwstate = SSL_READING;
            i = GMold_BIO_read(s->rbio, pkt + len + left, max - left);
        } else {
            i = -1;
        }
        ...
        if (i <= 0) {
            ...
            if (s->mode & SSL_MODE_RELEASE_BUFFERS && !SSL_IS_DTLS(s))
                if (len + left == 0)
                    ssl3_release_read_buffer(s);
            printf("GMold_ssl3 read_n 666666666666666666666666 i:%d\n",i);
            return i;
        }
        left += i;
        /*
...
int ssl3_write_bytes(SSL *s, int type, const void *buf_, size_t len,
                     size_t *written)
{
    printf("ddddddddddd ssl3_write_bytes \n");
...
int GMold_ssl3_write_bytes(SSL *s, int type, const void *buf_, int len, size_t *written)
{
    const unsigned char *buf = buf_;
    int tot;
     ...
     */
    if (wb->left != 0) {
        i = GMold_ssl3_write_pending(s, type, &buf[tot], s->rlayer.wpend_tot);
        if (i <= 0) {
            /* XXX should we ssl3_release_write_buffer if i<0? */
            s->rlayer.wnum = tot;
            return i;
    ...
    for (;;) {
        ...
        i = GMold_do_ssl3_write(s, type, &(buf[tot]), pipelens, numpipes, 0);
        if (i <= 0) {
            /* XXX should we ssl3_release_write_buffer if i<0? */
            s->rlayer.wnum = tot;
            return i;
...
int GMold_do_ssl3_write(SSL *s, int type, const unsigned char *buf,
                  ...
                  int create_empty_fragment)
{
    ...
    if (RECORD_LAYER_write_pending(&s->rlayer))
        return (GMold_ssl3_write_pending(s, type, buf, totlen));
    ...
    if (!clear && !create_empty_fragment && !s->s3->empty_fragment_done) {
        ...
        if (s->s3->need_empty_fragments && type == SSL3_RT_APPLICATION_DATA) {
            ...
            prefix_len = GMold_do_ssl3_write(s, type, buf, &tmppipelen, 1, 1);
            if (prefix_len <= 0)
                goto err;
    ...
    for (j = 0; j < numpipes; j++) {
        ...
        *(outbuf[j]++) = (s->version >> 8);
        ...
        if (!(s->s3->flags & TLS1_FLAGS_ENCRYPT_THEN_MAC) && mac_size != 0) {
...
int GMold_ssl3_write_pending(SSL *s, int type, const unsigned char *buf,
                       unsigned int len)
{
    int i;
...
{
    printf("ddddddddddd do_ssl3_write \n");
...
int ssl3_write_pending(SSL *s, int type, const unsigned char *buf, size_t len,
                       size_t *written)
{
    printf("ddddddddddd ssl3_write_pending \n");
    int i;
    SSL3_BUFFER *wb = s->rlayer.wbuf;
    size_t currbuf = 0;
    size_t tmpwrit = 0;
...
{
    printf("ddddddddddd ssl3_read_bytes \n");
    int i, j, ret;
...
int GMold_ssl3_read_bytes(SSL *s, int type, int *recvd_type, unsigned char *buf, int len, int peek, size_t *readbytes)
{
    int al, i, j, ret;
    ...
    if (!SSL3_BUFFER_is_initialised(rbuf)) {
        /* Not initialized yet */
        if (!GMold_ssl3_setup_read_buffer(s))
        {
            printf("GMold_ssl3 read_bytes 111111111111111111111111111\n");
            return (-1);
        }       
    }
    if ((type && (type != SSL3_RT_APPLICATION_DATA)
         && (type != SSL3_RT_HANDSHAKE)) || (peek
                                             && (type !=
                                                 SSL3_RT_APPLICATION_DATA))) 
    {
        printf("GMold_ssl3 read_bytes 222222222222222222222222222\n");
        return -1;
    }
    if ((type == SSL3_RT_HANDSHAKE) && (s->rlayer.handshake_fragment_len > 0))
        /* (partially) satisfy request from storage */
    {
        ...
        if (recvd_type != NULL)
            *recvd_type = SSL3_RT_HANDSHAKE;
        printf("GMold_ssl3 read_bytes 33333333333333333333333 n=%d\n",n);
        return n;
    }
    /*
     * Now s->rlayer.handshake_fragment_len == 0 if type == SSL3_RT_HANDSHAKE.
    ...
    if (!ossl_statem_get_in_handshake(s) && SSL_in_init(s)) {
        ...
        if (i < 0)
        {
            printf("GMold_ssl3 read_bytes 444444444444444 i=%d\n",i);
            return (i);
        }    
        if (i == 0) 
        {
            printf("GMold_ssl3 read_bytes 5555555555555555555 i==0\n");
            return (-1);
        }
    }
 start:
    ...
    do {
        /* get new records if necessary */
        if (num_recs == 0) {
            ret = GMold_ssl3_get_record(s);
            if (ret <= 0)
            {
                printf("GMold_ssl3 read_bytes 6666666666666666666 ret=%d\n",ret);
                return (ret);
            }           
            num_recs = RECORD_LAYER_get_numrpipes(&s->rlayer);
            if (num_recs == 0) {
                ...
                printf("GMold_ssl3 read_bytes 777777777777777777777777\n");
                goto f_err;
            }
        }
        /* Skip over any records we have already read */
    ...
    if (s->s3->change_cipher_spec /* set when we receive ChangeCipherSpec,
        ...
        printf("GMold_ssl3 read_bytes 8888888888888888888\n");
        goto f_err;
    }
    /*
     * If the other end has shut down, throw anything we read away (even in
    ...
    if (s->shutdown & SSL_RECEIVED_SHUTDOWN) {
        SSL3_RECORD_set_length(rr, 0);
        s->rwstate = SSL_NOTHING;
        printf("GMold_ssl3 read_bytes 9999999999999999999999\n");
        return (0);
    }
    if (type == SSL3_RECORD_get_type(rr)
        || (SSL3_RECORD_get_type(rr) == SSL3_RT_CHANGE_CIPHER_SPEC
        ...
        if (SSL_in_init(s) && (type == SSL3_RT_APPLICATION_DATA) &&
            (s->enc_read_ctx == NULL)) {
            al = SSL_AD_UNEXPECTED_MESSAGE;
            printf("GMold_ssl3 read_bytes aaaaaaaaaaaa\n");
            goto f_err;
        }
        if (type == SSL3_RT_HANDSHAKE
            && SSL3_RECORD_get_type(rr) == SSL3_RT_CHANGE_CIPHER_SPEC
            && s->rlayer.handshake_fragment_len > 0) {
            al = SSL_AD_UNEXPECTED_MESSAGE;
            printf("GMold_ssl3 read_bytes bbbbbbbbbbbbbbbbb\n");
            goto f_err;
        }
        if (recvd_type != NULL)
            *recvd_type = SSL3_RECORD_get_type(rr);
        if (len <= 0)
        {
            printf("GMold_ssl3 read_bytes cccccccccccccccccccc\n");
            return (len);
        }        
        read_bytes = 0;
        do {
        ...
        if (!peek && curr_rec == num_recs
            ...
            && SSL3_BUFFER_get_left(rbuf) == 0)
            ssl3_release_read_buffer(s);
        // printf("GMold_ssl3 read_bytes dddddddddddddddddddd\n");
        return read_bytes;
    }
    /*
     * If we get here, then type != rr->type; if we have a handshake message,
    ...
    if (rr->rec_version == SSL2_VERSION) {
         ...
         */
        al = SSL_AD_INTERNAL_ERROR;
        printf("GMold_ssl3 read_bytes eeeeeeeeeeeeeeeeeee\n");
        goto f_err;
    }
    if (s->method->version == TLS_ANY_VERSION
        && (s->server || rr->type != SSL3_RT_ALERT)) {
        ...
        s->version = rr->rec_version;
        al = SSL_AD_UNEXPECTED_MESSAGE;
        printf("GMold_ssl3 read_bytes fffffffffffffffffffff\n");
        goto f_err;
    }
    /*
     ...
     */
    {
        ...
        if (SSL3_RECORD_get_type(rr) == SSL3_RT_HANDSHAKE) {
            ...
            printf("GMold_ssl3_read_bytes 11 TODO xcs\n");
...
//             dest_len = &s->rlayer.alert_fragment_len;
        ...
        if (dest_maxlen > 0) {
            ...
            if (*dest_len < dest_maxlen) {
                SSL3_RECORD_set_read(rr);
                printf("GMold_ssl3 read_bytes ggggggggggggggggggggggggg\n");
                goto start;     /* fragment was too small */
            }
        }
    }
    ...
    if ((!s->server) &&
        ...
        if (SSL_is_init_finished(s) &&
            ...
            !s->s3->renegotiate) {
            ssl3_renegotiate(s);
            if (GMold_ssl3_renegotiate_check(s,1)) {
                i = s->handshake_func(s);
                if (i < 0)
                    return (i);
                if (i == 0) {
    ...
    printf("GMold_ssl3_read_bytes 22 TODO xcs\n");
...
//             s->rwstate = SSL_NOTHING;
//             s->s3->fatal_alert = alert_descr;
//             GMold_BIO_snprintf(tmp, sizeof tmp, "%d", alert_descr);
//             GMold_ERR_add_error_data(2, "SSL alert number ", tmp);
//             s->shutdown |= SSL_RECEIVED_SHUTDOWN;
//             SSL3_RECORD_set_read(rr);
//             SSL_CTX_remove_session(s->session_ctx, s->session);
//             return (0);
