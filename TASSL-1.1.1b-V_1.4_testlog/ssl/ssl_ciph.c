...
static const EVP_CIPHER *GMold_ssl_cipher_method = NULL;
...
{
    printf("ddddddddddd ssl_cipher_get_evp\n");
    int i;
    const SSL_CIPHER *c;
...
int GMold_ssl_cipher_get_evp(const SSL_SESSION *s, const EVP_CIPHER **enc,
                       const EVP_MD **md, int *mac_pkey_type,
                       int *mac_secret_size, SSL_COMP **comp, int use_etm)
{
    ...
    i = ssl_cipher_info_lookup(ssl_cipher_table_cipher, c->algorithm_enc);  //获取对称加密算法
    ...
    if (i == -1)
        *enc = NULL;
        ...
        if (i == SSL_ENC_NULL_IDX)
            ...
            *enc = GMold_EVP_sms4_cbc();
    }

    i = ssl_cipher_info_lookup(ssl_cipher_table_mac, c->algorithm_mac);     //获取对称身份验证算法
    if (i == -1) {
        *md = NULL;
        if (mac_pkey_type != NULL)
            *mac_pkey_type = NID_undef;
...
{
    printf("ddddddddd ssl_handshake_md\n");
    return ssl_md(ssl_get_algorithm2(s));
}

const EVP_MD *GMold_ssl_handshake_md(SSL *s)
{
    return ssl_md(SSL_PKEY_ECC_ENC);
}

const EVP_MD *ssl_prf_md(SSL *s)
{
    printf("ddddddddddd ssl_prf_md \n");
    return ssl_md(ssl_get_algorithm2(s) >> TLS1_PRF_DGST_SHIFT);
}

const EVP_MD *GMold_ssl_prf_md(SSL *s)
{
    return GMold_EVP_sm3();
}
...
int GMold_ssl_cipher_get_cert_index(const SSL_CIPHER *c)
{
    uint32_t alg_a;
    ...
    if (alg_a & SSL_aECDSA)
        ...
        return GMold_SSL_PKEY_GOST_EC;
    else if (alg_a & SSL_aGOST01)
        return SSL_PKEY_GOST01;
    else if (alg_a & GMold_SSL_aSM2)
...
const SSL_CIPHER *ssl_get_cipher_by_char(SSL *ssl, const unsigned char *ptr,
                                         int all)
{
    printf("ddddddd ssl_get_cipher_by_char\n");
...
const SSL_CIPHER *GMold_ssl_get_cipher_by_char(SSL *ssl, const unsigned char *ptr)
{
    return ssl->method->get_cipher(0);
}
