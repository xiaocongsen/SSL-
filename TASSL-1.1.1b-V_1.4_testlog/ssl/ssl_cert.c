...
static int ssl_security_default_callback(const SSL *s, const SSL_CTX *ctx,
                                         ...
                                         void *ex);
static int GMold_ssl_security_default_callback(const SSL *s, const SSL_CTX *ctx,
                                         int op, int bits, int nid, void *other,
                                         void *ex);
...
int GMold_ssl_add_cert_to_buf(BUF_MEM *buf, unsigned long *l, X509 *x)
{
    ...
    n = GMold_i2d_X509(x, NULL);
    if (n < 0 || !BUF_MEM_grow_clean(buf, (int)(n + (*l) + 3))) {
        return 0;
    }
    p = (unsigned char *)&(buf->data[*l]);
    l2n3(n, p);
    n = GMold_i2d_X509(x, &p);
    if (n < 0) {
        /* Shouldn't happen */
        return 0;
    }
...
CERT *GMold_ssl_cert_new(void)
{
    ...
    ret->key = &(ret->pkeys[SSL_PKEY_RSA]);
    ret->references = 1;
    ret->sec_cb = GMold_ssl_security_default_callback;
    ret->sec_level = OPENSSL_TLS_SECURITY_LEVEL;
    ret->sec_ex = NULL;
    ret->lock = CRYPTO_THREAD_lock_new();
    if (ret->lock == NULL) {
...
{
    printf("ddddddddddd ssl_verify_cert_chain\n");
    X509 *x;
    int i = 0;
...
int GMold_ssl_verify_cert_chain(SSL *s, STACK_OF(X509) *sk)
{
    X509 *x;
    int i = 0;
    X509_STORE *verify_store;
    ...
    x = sk_X509_value(sk, 0);
    if (!GMold_X509_STORE_CTX_init(ctx, verify_store, x, sk)) {
        goto end;
    }
    param = X509_STORE_CTX_get0_param(ctx);
    /*
...
static int ssl_security_default_callback(const SSL *s, const SSL_CTX *ctx,
                                         ...
                                         void *ex)
{
    printf("ddddddddddd ssl_security_default_callback \n");
    int level, minbits;
...
{
    printf("ddddddddddd ssl_security \n");
...
static int GMold_ssl_security_default_callback(const SSL *s, const SSL_CTX *ctx,
                                         ...
                                         void *ex)
{
    // printf("GMold_ssl_security_default_callback 111111111111111111111\n");
    int level, minbits;
    static const int minbits_table[5] = { 80, 112, 128, 192, 256 };
    if (ctx)
        level = SSL_CTX_get_security_level(ctx);
    ...
    case SSL_SECOP_CIPHER_CHECK:
        {
            ...
            if (level >= 3 && !(c->algorithm_mkey &
                (SSL_kEDH | SSL_kEECDH | SSL_kSM2DH)))
                return 0;
            break;
        }
    case SSL_SECOP_VERSION:
...
int GMold_ssl_security(const SSL *s, int op, int bits, int nid, void *other)
{
    return GMold_ssl_security_default_callback(s, NULL, op, bits, nid, other, s->cert->sec_ex);
}
