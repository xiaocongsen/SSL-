...
#  define GMold_SSL_aSM2               0x00000100U
...
# define GMold_SSL_PKEY_GOST_EC SSL_PKEY_NUM+1
...
struct ssl_cipher_st {
     ...
     * 'algorithms'
     */
    uint32_t algorithm_mkey;    /* key exchange algorithm 密钥交换算法*/
    uint32_t algorithm_auth;    /* server authentication 服务器身份验证*/
    uint32_t algorithm_enc;     /* symmetric encryption 对称加密*/
    uint32_t algorithm_mac;     /* symmetric authentication 对称身份验证*/
...
# endif
        ...
        /* Temporary storage for premaster secret 主密钥前的临时存储*/
        unsigned char *pms;
        size_t pmslen;
# ifndef OPENSSL_NO_PSK
...
__owur CERT *GMold_ssl_cert_new(void);
...
__owur int ssl_cipher_get_evp(const SSL_SESSION *s, const EVP_CIPHER **enc,
                              ...
                              int use_etm);
__owur int GMold_ssl_cipher_get_evp(const SSL_SESSION *s, const EVP_CIPHER **enc,
                       const EVP_MD **md, int *mac_pkey_type,
...
__owur int GMold_ssl_cipher_get_cert_index(const SSL_CIPHER *c);
__owur const SSL_CIPHER *ssl_get_cipher_by_char(SSL *ssl,
                                                const unsigned char *ptr,
                                                int all);
__owur   const SSL_CIPHER *GMold_ssl_get_cipher_by_char(SSL *ssl, const unsigned char *ptr);
...
__owur int GMold_ssl_verify_cert_chain(SSL *s, STACK_OF(X509) *sk);
__owur int ssl_build_cert_chain(SSL *s, SSL_CTX *ctx, int flags);
__owur int ssl_cert_set_cert_store(CERT *c, X509_STORE *store, int chain,
                                   int ref);

__owur int ssl_security(const SSL *s, int op, int bits, int nid, void *other);
__owur int GMold_ssl_security(const SSL *s, int op, int bits, int nid, void *other);
...
__owur int GMold_ssl_verify_alarm_type(long type);
void ssl_sort_cipher_list(void);
int ssl_load_ciphers(void);
__owur int ssl_fill_hello_random(SSL *s, int server, unsigned char *field,
                                 size_t len, DOWNGRADE dgrd);
__owur int GMold_ssl_fill_hello_random(SSL *s, int server, unsigned char *result, int len);
__owur int ssl_generate_master_secret(SSL *s, unsigned char *pms, size_t pmslen,
                                      int free_pms);
__owur int GMold_ssl_generate_master_secret(SSL *s, unsigned char *pms, size_t pmslen,
                                      int free_pms);
...
__owur int ssl3_put_cipher_by_char(const SSL_CIPHER *c, WPACKET *pkt,
                                   size_t *len);
__owur int GMold_ssl3_put_cipher_by_char(const SSL_CIPHER *c, unsigned char *p);
int ssl3_init_finished_mac(SSL *s);
int GMold_ssl3_init_finished_mac(SSL *s);
__owur int ssl3_setup_key_block(SSL *s);
__owur int ssl3_change_cipher_state(SSL *s, int which);
void ssl3_cleanup_key_block(SSL *s);
__owur int ssl3_do_write(SSL *s, int type);
__owur int GMold_ssl3_do_write(SSL *s, int type);
int ssl3_send_alert(SSL *s, int level, int desc);
int GMold_ssl3_send_alert(SSL *s, int level, int desc);
__owur int ssl3_generate_master_secret(SSL *s, unsigned char *out,
                                       unsigned char *p, size_t len,
                                       size_t *secret_size);
__owur int ssl3_get_req_cert_type(SSL *s, WPACKET *pkt);
__owur int ssl3_num_ciphers(void);
__owur int GMold_ssl3_num_ciphers(void);
__owur const SSL_CIPHER *ssl3_get_cipher(unsigned int u);
__owur const SSL_CIPHER *GMold_ssl3_get_cipher(unsigned int u);
int ssl3_renegotiate(SSL *ssl);
int GMold_ssl3_renegotiate(SSL *ssl);
int ssl3_renegotiate_check(SSL *ssl, int initok);
int GMold_ssl3_renegotiate_check(SSL *ssl, int initok);
__owur int ssl3_dispatch_alert(SSL *s);
__owur int GMold_ssl3_dispatch_alert(SSL *s);
...
__owur int GMold_ssl3_finish_mac(SSL *s, const unsigned char *buf, int len);
...
__owur int GMold_ssl3_digest_cached_records(SSL *s, int keep);
__owur int ssl3_new(SSL *s);
void ssl3_free(SSL *s);
__owur int ssl3_read(SSL *s, void *buf, size_t len, size_t *readbytes);
__owur int GMold_ssl3_read(SSL *s, void *buf, int len, size_t *readbytes);
...
__owur int GMold_ssl3_write(SSL *s, const void *buf, int len);
__owur int ssl3_shutdown(SSL *s);
__owur int GMold_ssl3_shutdown(SSL *s);
int ssl3_clear(SSL *s);
__owur long ssl3_ctrl(SSL *s, int cmd, long larg, void *parg);
__owur long ssl3_ctx_ctrl(SSL_CTX *s, int cmd, long larg, void *parg);
__owur long GMold_ssl3_ctx_ctrl(SSL_CTX *ctx, int cmd, long larg, void *parg);
...
__owur int GMold_ssl3_set_handshake_header(SSL *s, int htype, unsigned long len);
...
__owur int GMold_ssl3_handshake_write(SSL *s);

__owur int ssl_allow_compression(SSL *s);
__owur int GMold_ssl_allow_compression(SSL *s);
__owur int ssl_version_supported(const SSL *s, int version,
                                 const SSL_METHOD **meth);
__owur int GMold_ssl_version_supported(const SSL *s, int version);
...
__owur int GMold_ssl_choose_client_version(SSL *s, int version);
__owur int ssl_get_min_max_version(const SSL *s, int *min_version,
                                   int *max_version, int *real_max);
int GMold_ssl_get_client_min_max_version(const SSL *s, int *min_version,
                                   int *max_version);
...
__owur int GMold_ssl_init_wbio_buffer(SSL *s);
int ssl_free_wbio_buffer(SSL *s);
void GMold_ssl_free_wbio_buffer(SSL *s);

__owur int tls1_change_cipher_state(SSL *s, int which);
__owur int GMold_tls1_change_cipher_state(SSL *s, int which);
__owur int tls1_setup_key_block(SSL *s);
__owur int GMold_tls1_setup_key_block(SSL *s);
__owur size_t tls1_final_finish_mac(SSL *s, const char *str, size_t slen,
                                    unsigned char *p);
__owur size_t GMold_tls1_final_finish_mac(SSL *s, const char *str, size_t slen, unsigned char *out);
__owur int tls1_generate_master_secret(SSL *s, unsigned char *out,
                                       unsigned char *p, size_t len,
                                       size_t *secret_size);
__owur int GMold_tls1_generate_master_secret(SSL *s, unsigned char *out,
                                       unsigned char *p, size_t len,
                                       size_t *secret_size);
...
__owur int tls1_export_keying_material(SSL *s, unsigned char *out, size_t olen,
                                       ...
                                       int use_context);
__owur int GMold_tls1_export_keying_material(SSL *s, unsigned char *out, size_t olen,
                                       const char *label, size_t llen,
                                       const unsigned char *p, size_t plen,
                                       int use_context);
...
__owur int GMold_tls1_set_curves(unsigned char **pext, size_t *pextlen,
                    int *curves, size_t ncurves);
__owur int GMold_tls1_set_curves_list(unsigned char **pext, size_t *pextlen, const char *str);
...
__owur int GMold_tls12_get_sigandhash(unsigned char *p, const EVP_PKEY *pk, const EVP_MD *md);
...
__owur int GMold_tls1_set_sigalgs_list(CERT *c, const char *str, int client);
...
__owur int GMold_ssl_security_cert(SSL *s, SSL_CTX *ctx, X509 *x, int vfy, int is_ee);
__owur int ssl_security_cert_chain(SSL *s, STACK_OF(X509) *sk, X509 *ex,
                                   int vfy);
__owur int GMold_ssl_security_cert_chain(SSL *s, STACK_OF(X509) *sk, X509 *x, int vfy);
...
__owur size_t GMold_tls12_get_psigalgs(SSL *s, const unsigned char **psigs);
#  ifndef OPENSSL_NO_EC
__owur int tls_check_sigalg_curve(const SSL *s, int curve);
#  endif
__owur int tls12_check_peer_sigalg(SSL *s, uint16_t, EVP_PKEY *pkey);
__owur int ssl_set_client_disabled(SSL *s);
__owur void GMold_ssl_set_client_disabled(SSL *s);
__owur int ssl_cipher_disabled(SSL *s, const SSL_CIPHER *c, int op, int echde);
__owur int GMold_ssl_cipher_disabled(SSL *s, const SSL_CIPHER *c, int op, int echde);
__owur int ssl_handshake_hash(SSL *s, unsigned char *out, size_t outlen,
                                 size_t *hashlen);
__owur int GMold_ssl_handshake_hash(SSL *s, unsigned char *out, int outlen);
__owur const EVP_MD *ssl_md(int idx);
__owur const EVP_MD *ssl_handshake_md(SSL *s);
__owur const EVP_MD *GMold_ssl_handshake_md(SSL *s);
__owur const EVP_MD *ssl_prf_md(SSL *s);
__owur const EVP_MD *GMold_ssl_prf_md(SSL *s);
