...
int GMold_ssl_add_cert_to_buf(BUF_MEM *buf, unsigned long *l, X509 *x);
...
__owur int GMold_tls_get_message_header(SSL *s, int *mt);
__owur int tls_get_message_body(SSL *s, size_t *len);
__owur int GMold_tls_get_message_body(SSL *s, size_t *len);
...
__owur int GMold_tls_construct_change_cipher_spec(SSL *s);

__owur int tls_construct_finished(SSL *s, WPACKET *pkt);
__owur int GMold_tls_construct_finished(SSL *s, const char *sender, int slen);
...
__owur WORK_STATE tls_finish_handshake(SSL *s, WORK_STATE wst, int clearbufs,
                                       int stop);
__owur WORK_STATE GMold_tls_finish_handshake(SSL *s, WORK_STATE wst);
__owur WORK_STATE dtls_wait_for_dry(SSL *s);

/* some client-only functions */
__owur int tls_construct_client_hello(SSL *s, WPACKET *pkt);
__owur int GMold_tls_construct_client_hello(SSL *s);
__owur int GMold_construct_client_certificate(SSL *s);
__owur int GMold_output_cert_chain(SSL *s, int *len, int a_idx, int k_idx);
__owur int GMold_construct_client_key_exchange(SSL *s);
__owur int GMold_tls_construct_client_verify(SSL *s);
__owur int GMold_process_ske_sm2(SSL *s, PACKET *pkt, int *al);
__owur unsigned char *GMold_new_cert_packet(X509 *x, int *l);

__owur MSG_PROCESS_RETURN tls_process_server_hello(SSL *s, PACKET *pkt);
__owur MSG_PROCESS_RETURN GMold_tls_process_server_hello(SSL *s, PACKET *pkt);
...
__owur MSG_PROCESS_RETURN GMold_tls_process_server_done(SSL *s, PACKET *pkt);
...
__owur WORK_STATE GMold_tls_prepare_client_certificate(SSL *s, WORK_STATE wst);
...
__owur int GMold_tls_client_key_exchange_post_work(SSL *s);
...
__owur MSG_PROCESS_RETURN GMold_process_server_key_exchange(SSL *s, PACKET *pkt);
__owur MSG_PROCESS_RETURN tls_process_server_certificate(SSL *s, PACKET *pkt);
__owur MSG_PROCESS_RETURN GMold_tls_process_server_certificate(SSL *s, PACKET *pkt);
__owur int ssl3_check_cert_and_algorithm(SSL *s);
#ifndef OPENSSL_NO_NEXTPROTONEG
__owur int tls_construct_next_proto(SSL *s, WPACKET *pkt);
__owur int GMold_tls_construct_next_proto(SSL *s);
#endif
