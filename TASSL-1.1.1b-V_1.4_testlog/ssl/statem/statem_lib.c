...
{
    printf("ddddddddddd ssl3_do_write \n");
...
int GMold_ssl3_do_write(SSL *s, int type)
{
    int ret;
    ret = GMold_ssl3_write_bytes(s, type, &s->init_buf->data[s->init_off],
                           s->init_num,NULL);
    if (ret < 0)
    {
        printf("ssl3_do_write xcs 11111111 \n");
        return (-1);
    }
    if (type == SSL3_RT_HANDSHAKE)
        /*
         ...
         * ignore the result anyway
         */
        if (!GMold_ssl3_finish_mac(s,
                             (unsigned char *)&s->init_buf->data[s->init_off],
                             ret))
                             {
                                 printf("ssl3_do_write xcs 333333333333333 \n");
                                return -1;
                             }
    ...
    if (ret == s->init_num) {
        if (s->msg_callback)
            s->msg_callback(1, s->version, type, s->init_buf->data,
                            (size_t)(s->init_off + s->init_num), s,
                            s->msg_callback_arg);
        printf("ssl3_do_write xcs 555555555555555555555 \n");
        return (1);
    }
    s->init_off += ret;
    s->init_num -= ret;
...
{
    printf("ddddddddddd tls_close_construct_packet \n");
    size_t msglen;
...
{
    printf("ddddddddddd tls_construct_finished \n");
...
int GMold_tls_construct_finished(SSL *s, const char *sender, int slen)
{
    unsigned char *p;
    int i;
    unsigned long l;
    ...
    if (!GMold_ssl3_set_handshake_header(s, SSL3_MT_FINISHED, l)) {
        return 0;
    }
...
{
    printf("ddddddddddd ssl3_output_cert_chain \n");
...
{
    printf("ddddddddddd tls_finish_handshake \n");
...
WORK_STATE GMold_tls_finish_handshake(SSL *s, WORK_STATE wst)
{
    void (*cb) (const SSL *ssl, int type, int val) = NULL;
    ...
    if (!s->server || s->renegotiate == 2) {
        ...
        s->handshake_func = GMold_ossl_statem_connect;
...
{
    printf("ddddddddddd tls_get_message_header \n");
...
int GMold_tls_get_message_header(SSL *s, int *mt)
{
    /* s->init_num < SSL3_HM_HEADER_LENGTH */
    ...
    do {
        while (s->init_num < SSL3_HM_HEADER_LENGTH) {
            i = GMold_ssl3_read_bytes(s, SSL3_RT_HANDSHAKE, &recvd_type,
                                          &p[s->init_num],
                                          SSL3_HM_HEADER_LENGTH - s->init_num,
                                          0,NULL);
            if (i <= 0) {
                // printf("GMold_tls_get message_header 1111111111111111111111111111111\n");
                s->rwstate = SSL_READING;
                return 0;
            }
            if (recvd_type == SSL3_RT_CHANGE_CIPHER_SPEC) {
                ...
                if (s->init_num != 0 || i != 1 || p[0] != SSL3_MT_CCS) {
                    ...
                    printf("GMold_tls_get message_header 222222222222222222222\n");
                    goto f_err;
                }
                s->s3->tmp.message_type = *mt = SSL3_MT_CHANGE_CIPHER_SPEC;
                s->init_num = i - 1;
                ...
                al = SSL_AD_UNEXPECTED_MESSAGE;
                printf("GMold_tls_get message_header 333333333333333333333333333333\n");
                goto f_err;
            }
            s->init_num += i;
        }
    ...
    *mt = *p;
    ...
    if (RECORD_LAYER_is_sslv2_record(&s->rlayer)) {
        ...
        n2l3(p, l);
        /* GMold_BUF_MEM_grow takes an 'int' parameter */
        if (l > (INT_MAX - SSL3_HM_HEADER_LENGTH)) {
            al = SSL_AD_ILLEGAL_PARAMETER;
            printf("GMold_tls_get message_header 4444444444444444444444444444444\n");
            goto f_err;
        }
        s->s3->tmp.message_size = l;
...
{
    printf("ddddddddddd tls_get_message_body \n");
    ...
    if (RECORD_LAYER_is_sslv2_record(&s->rlayer)) 
    {
        printf("ddddddd 11111111111111111111111111111111111\n");
        ...
        if (!SSL_IS_TLS13(s) || (s->s3->tmp.message_type != SSL3_MT_NEWSESSION_TICKET
            ...
            if (s->s3->tmp.message_type != SSL3_MT_SERVER_HELLO
                    ...
                    || memcmp(hrrrandom,
                              ...
                              SSL3_RANDOM_SIZE) != 0) {
                printf("ddddddd 2222222222222222222222222\n");
                if (!ssl3_finish_mac(s, (unsigned char *)s->init_buf->data,
                                     s->init_num + SSL3_HM_HEADER_LENGTH)) {
                    /* SSLfatal() already called */
                    *len = 0;
...
int GMold_tls_get_message_body(SSL *s, size_t *len)
{
    long n;
    unsigned char *p;
    int i;
    ...
    n = s->s3->tmp.message_size - s->init_num;
    while (n > 0) {
        i = GMold_ssl3_read_bytes(s, SSL3_RT_HANDSHAKE, NULL,
                                      &p[s->init_num], n, 0,NULL);
        if (i <= 0) {
            s->rwstate = SSL_READING;
            *len = 0;
            return 0;
    ...
    /* Feed this message into MAC computation. */
    if (RECORD_LAYER_is_sslv2_record(&s->rlayer)) {
        if (!GMold_ssl3_finish_mac(s, (unsigned char *)s->init_buf->data,
                             s->init_num)) {
...
{
    printf("ddddddddddd ssl_allow_compression \n");
    if (s->options & SSL_OP_NO_COMPRESSION)
        return 0;
    return ssl_security(s, SSL_SECOP_COMPRESSION, 0, 0, NULL);
}

int GMold_ssl_allow_compression(SSL *s)
{
    if (s->options & SSL_OP_NO_COMPRESSION)
        return 0;
    return GMold_ssl_security(s, SSL_SECOP_COMPRESSION, 0, 0, NULL);
}
...
static const version_info tls_version_table[] = {
    ...
    {GMTLS_VERSION, gmtls_client_method, tlsv1_1_server_method},
#else
...
{
    printf("ddddddddddd ssl_version_supported \n");
...
int GMold_ssl_version_supported(const SSL *s, int version)
{
    const version_info *vent;
    const version_info *table;
...
{
    printf("ddddddddddd ssl_choose_client_version \n");
...
int GMold_ssl_choose_client_version(SSL *s, int version)
{
    const version_info *vent;
    const version_info *table;
                                  ...
                                  size_t hrrlen)
{
    ...
    printf("ddddddd 555555555555555555555\n");
     ...
     */
    printf("ddddddd 6666666666666666666\n");
    if (hrr != NULL
