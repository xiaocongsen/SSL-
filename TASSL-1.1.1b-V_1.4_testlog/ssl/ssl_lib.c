...
static int GMold_ssl_cert_type_ecc(const X509 *x, const EVP_PKEY *pk)
{
    if (x && GMold_X509_get_signature_nid(x) == GMold_NID_sm2sign_with_sm3) {
        if (GMold_X509_get_key_usage((X509 *)x) & X509v3_KU_DIGITAL_SIGNATURE)
            return GMold_SSL_PKEY_SM2;
        else
            return SSL_PKEY_ECC_ENC;
    }
    if (EC_GROUP_get_curve_name(EC_KEY_get0_group(
        (EC_KEY *)EVP_PKEY_get0(pk))) == GMold_NID_sm2p256v1) {
        return GMold_SSL_PKEY_SM2;
    }
    return SSL_PKEY_ECC;
}
...
{
    if (pk == NULL && (pk = GMold_X509_get0_pubkey(x)) == NULL)
        return -1;

    switch (EVP_PKEY_id(pk)) {
    default:
        return -1;
    case GMold_NID_rsaEncryption:
        return SSL_PKEY_RSA_ENC;
    case GMold_NID_dsa:
        return SSL_PKEY_DSA_SIGN;
    case GMold_NID_X9_62_id_ecPublicKey:
        return GMold_ssl_cert_type_ecc(x, pk);
    case GMold_NID_id_GostR3410_2001:
        return SSL_PKEY_GOST01;
    case GMold_NID_id_GostR3410_2012_256:
        return SSL_PKEY_GOST12_256;
    case GMold_NID_id_GostR3410_2012_512:
...
{
    if (s->method == NULL) {
        printf("SSL_clear 11111111111111111111111111\n");
    ...
    if (s->renegotiate) {
        printf("SSL_clear 22222222222222222222222222\n");
...
int GMold_SSL_set_fd(SSL *s, int fd)
{
    int ret = 0;
    BIO *bio = NULL;

    bio = BIO_new(GMold_BIO_s_socket());

    if (bio == NULL) {
        goto err;
    }
    GMold_BIO_int_ctrl(bio,BIO_C_SET_FD,BIO_NOCLOSE,fd);
    SSL_set_bio(s, bio, bio);
    ret = 1;
 err:
    return (ret);
...
int GMold_SSL_CTX_check_private_key(const SSL_CTX *ctx)
{
    if ((ctx == NULL) || (ctx->cert->key->x509 == NULL)) {
        return (0);
    }
    ...
    return (GMold_X509_check_private_key(ctx->cert->key->x509, ctx->cert->key->privatekey));
}
...
int GMold_SSL_read(SSL *s, void *buf, int num)
{
    if (s->handshake_func == NULL) {
        return -1;
    }
    ...
    if ((s->mode & SSL_MODE_ASYNC) && ASYNC_get_current_job() == NULL) {
        ...
        return s->method->ssl_read(s, buf, num,NULL);
    }
}
...
int GMold_SSL_write(SSL *s, const void *buf, int num)
{
    if (s->handshake_func == NULL) {
        return -1;
    }
    ...
    if ((s->mode & SSL_MODE_ASYNC) && ASYNC_get_current_job() == NULL) {
        ...
        return s->method->ssl_write(s, buf, num,NULL);
    }
}
...
int SSL_CTX_set_cipher_list(SSL_CTX *ctx, const char *str)
{
     ...
     */
    printf("SSL_CTX_set_cipher_list: sk_SSL_CIPHER_num:%d, cipher_list_tls12_num:%d\n",sk_SSL_CIPHER_num(sk),cipher_list_tls12_num(sk));
    if (sk == NULL)
        return 0;
...
SSL_CTX *GMold_SSL_CTX_new(const SSL_METHOD *meth)
{
    SSL_CTX *ret = NULL;
    ...
    ret->max_cert_list = SSL_MAX_CERT_LIST_DEFAULT;
    ret->verify_mode = SSL_VERIFY_NONE;
    if ((ret->cert = GMold_ssl_cert_new()) == NULL)
        goto err;
    ...
    /* Setup RFC5077 ticket keys */
    if ((GMold_RAND_bytes(ret->ext.tick_key_name,
                    sizeof(ret->ext.tick_key_name)) <= 0)
        || (GMold_RAND_bytes(ret->ext.secure->tick_hmac_key,
                       sizeof(ret->ext.secure->tick_hmac_key)) <= 0)
        || (GMold_RAND_bytes(ret->ext.secure->tick_aes_key,
                       sizeof(ret->ext.secure->tick_aes_key)) <= 0))
        ret->options |= SSL_OP_NO_TICKET;
...
{
    printf("dddddddd ssl_init_wbio_buffer\n");
    BIO *bbio;
...
int GMold_ssl_init_wbio_buffer(SSL *s)
{
    BIO *bbio;
    ...
    bbio = BIO_new(GMold_BIO_f_buffer());
    if (bbio == NULL || !GMold_BIO_int_ctrl(bbio,BIO_C_SET_BUFF_SIZE,1,0)) {
        BIO_free(bbio);
        return 0;
    }
    s->bbio = bbio;
    s->wbio = GMold_BIO_push(bbio, s->wbio);
...
int GMold_SSL_CTX_load_verify_locations(SSL_CTX *ctx, const char *CAfile,
                                  const char *CApath)
{
    return (GMold_X509_STORE_load_locations(ctx->cert_store, CAfile, CApath));
}
...
int ssl_handshake_hash(SSL *s, unsigned char *out, size_t outlen,
                       size_t *hashlen)
{
    printf("ddddddddddd ssl_handshake_hash \n");
    EVP_MD_CTX *ctx = NULL;
...
/* Retrieve handshake hashes */
int GMold_ssl_handshake_hash(SSL *s, unsigned char *out, int outlen)
{
    EVP_MD_CTX *ctx = NULL;
    EVP_MD_CTX *hdgst = s->s3->handshake_dgst;
