...
{
    printf("ddddddddddddddddd ssl3_init_finished_mac\n");
...
int GMold_ssl3_init_finished_mac(SSL *s)
{
    ...
    ssl3_free_digest_list(s);
    s->s3->handshake_buffer = buf;
    (void)GMold_BIO_set_close(s->s3->handshake_buffer, BIO_CLOSE);
    return 1;
}
...
{
    int ret;
    printf("ddddddddddd ssl3_finish_mac \n");
...
int GMold_ssl3_finish_mac(SSL *s, const unsigned char *buf, int len)
{
    if (s->s3->handshake_dgst == NULL)
        /* Note: this writes to a memory BIO so a failure is a fatal error */
        return GMold_BIO_write(s->s3->handshake_buffer, (void *)buf, len) == len;
    else
...
int ssl3_digest_cached_records(SSL *s, int keep)
{
    ...
    long hdatalen;
    void *hdata;
    printf("ddddddddddd ssl3_digest_cached_records \n");
...
int GMold_ssl3_digest_cached_records(SSL *s, int keep)
{
    const EVP_MD *md;
    long hdatalen;
    void *hdata;
    ...
    if (s->s3->handshake_dgst == NULL) {
        hdatalen = GMold_BIO_ctrl(s->s3->handshake_buffer,BIO_CTRL_INFO,0,&hdata);
        printf("ssl3_digest_cached_records 1111 hdatalen:%d\n",hdatalen);
        if (hdatalen <= 0) 
        {
            return 0;
        }
        ...
        md = GMold_ssl_handshake_md(s);
        if (md == NULL || !GMold_EVP_DigestInit_ex(s->s3->handshake_dgst, md, NULL)
            || !GMold_EVP_DigestUpdate(s->s3->handshake_dgst, hdata, hdatalen)) 
        {
            return 0;
        }
    }
