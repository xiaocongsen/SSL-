...
static int GMold_ssl_set_cert(CERT *c, X509 *x);
...
static int GMold_ssl_set_pkey(CERT *c, EVP_PKEY *pkey)
{
    int i;

    i = GMold_ssl_cert_type(NULL, pkey);
    if (i < 0) {
        return (0);
    }

    if (i == GMold_SSL_PKEY_SM2 && c->pkeys[SSL_PKEY_ECC_ENC].x509)
        i = SSL_PKEY_ECC_ENC;

    if (c->pkeys[i].x509 != NULL) {
        EVP_PKEY *pktmp;
        pktmp = GMold_X509_get0_pubkey(c->pkeys[i].x509);
        if (pktmp == NULL) {
            return 0;
        }
        /*
         ...
         * cards.
         */
        if (EVP_PKEY_id(pkey) == GMold_EVP_PKEY_RSA
            && RSA_flags(EVP_PKEY_get0_RSA(pkey)) & RSA_METHOD_FLAG_NO_CHECK) ;
        else
        if (!GMold_X509_check_private_key(c->pkeys[i].x509, pkey)) {
            X509_free(c->pkeys[i].x509);
            c->pkeys[i].x509 = NULL;
            return 0;
        }
...
{
    printf("ddddddddddddddddd SSL_CTX_use_certificate\n");
    int rv;
    if (x == NULL) {
...
int GMold_SSL_CTX_use_certificate(SSL_CTX *ctx, X509 *x)
{
    int rv;
    if (x == NULL) {
        return (0);
    }
    rv = GMold_ssl_security_cert(NULL, ctx, x, 0, 1);
    if (rv != 1) {
        return 0;
    }
    return (GMold_ssl_set_cert(ctx->cert, x));
}
...
static int GMold_ssl_set_cert(CERT *c, X509 *x)
{
    ...
    if (c->pkeys[i].privatekey != NULL) {
       /*
         ...
         * cards.
         */
        if (EVP_PKEY_id(c->pkeys[i].privatekey) == GMold_EVP_PKEY_RSA
            && RSA_flags(EVP_PKEY_get0_RSA(c->pkeys[i].privatekey)) &
            RSA_METHOD_FLAG_NO_CHECK) ;
        else
        if (!X509_check_private_key(x, c->pkeys[i].privatekey)) {
...
{
    printf("ddddddddddddddddd SSL_CTX_use_certificate_file\n");
    int j;
    BIO *in;
    int ret = 0;
    X509 *x = NULL;
...
int GMold_SSL_CTX_use_certificate_file(SSL_CTX *ctx, const char *file, int type)
{
    int j;
    BIO *in;
    int ret = 0;
    ...
    if (GMold_BIO_ctrl(in,BIO_C_SET_FILENAME, BIO_CLOSE|BIO_FP_READ,(char *)(file)) <= 0) {
        goto end;
    }
    if (type == SSL_FILETYPE_ASN1) {
        j = ERR_R_ASN1_LIB;
        ...
        j = ERR_R_PEM_LIB;
        x = GMold_PEM_read_bio_X509(in, NULL, ctx->default_passwd_callback,
                              ctx->default_passwd_callback_userdata);
    } else {
        goto end;
    }
    ...
    ret = GMold_SSL_CTX_use_certificate(ctx, x);
 end:
    X509_free(x);
    BIO_free(in);
    return (ret);
...
int GMold_SSL_CTX_use_PrivateKey(SSL_CTX *ctx, EVP_PKEY *pkey)
{
    if (pkey == NULL) {
        return (0);
    }
    return (GMold_ssl_set_pkey(ctx->cert, pkey));
}
...
int GMold_SSL_CTX_use_PrivateKey_file(SSL_CTX *ctx, const char *file, int type)
{
    int j, ret = 0;
    BIO *in;
    EVP_PKEY *pkey = NULL;
    ...
    if (type == SSL_FILETYPE_PEM) {
        j = ERR_R_PEM_LIB;
        pkey = GMold_PEM_read_bio_PrivateKey(in, NULL,
                                       ctx->default_passwd_callback,
                                       ctx->default_passwd_callback_userdata);
    } else if (type == SSL_FILETYPE_ASN1) {
        j = ERR_R_ASN1_LIB;
    ...
    ret = GMold_SSL_CTX_use_PrivateKey(ctx, pkey);
    EVP_PKEY_free(pkey);
 end:
    BIO_free(in);
    return (ret);
