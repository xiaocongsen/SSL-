...
int GMold_ssl_verify_alarm_type(long type)
{
    int al;
...
{
    printf("dddddddddd ssl3_num_ciphers\n");
    return SSL3_NUM_CIPHERS;
}

int GMold_ssl3_num_ciphers(void)
{
    return 1;
}
...
{
    printf("dddddddddd ssl3_get_cipher\n");
...
const SSL_CIPHER *GMold_ssl3_get_cipher(unsigned int u)
{
    static const SSL_CIPHER tmp=
    {
        1,
        "SM2-WITH-SMS4-SM3",
        NULL,
...
{
    printf("ddddddddddd ssl3_set_handshake_header \n");
...
int GMold_ssl3_set_handshake_header(SSL *s, int htype, unsigned long len)
{
...
{
    printf("dddddddddd ssl3_handshake_write\n");
    return ssl3_do_write(s, SSL3_RT_HANDSHAKE);
}

int GMold_ssl3_handshake_write(SSL *s)
{
...
{
    printf("dddddddddd ssl3_ctx_ctrl\n");
    switch (cmd) {
#ifndef OPENSSL_NO_DH
    case SSL_CTRL_SET_TMP_DH:
        {
...
long GMold_ssl3_ctx_ctrl(SSL_CTX *ctx, int cmd, long larg, void *parg)
{
    switch (cmd) {
    case SSL_CTRL_SET_TMP_DH:
        {
    ...
    case SSL_CTRL_SET_TMP_ECDH:
        {
            ...
            if (nid == GMold_NID_undef)
                return 0;
            return GMold_tls1_set_curves(&ctx->ext.supportedgroups,
                                   &ctx->ext.supportedgroups_len,
                                   &nid, 1);
        }
        /* break; */
    ...
    case SSL_CTRL_GET_TLSEXT_TICKET_KEYS:
        {
            unsigned char *keys = parg;
            long tlsext_tick_keylen = (sizeof(ctx->ext.tick_key_name) +
                                       sizeof(ctx->ext.secure->tick_hmac_key) +
                                       sizeof(ctx->ext.secure->tick_aes_key));
            if (keys == NULL)
    ...
    case SSL_CTRL_SET_CURVES_LIST:
        return GMold_tls1_set_curves_list(&ctx->ext.supportedgroups,
                                    &ctx->ext.supportedgroups_len,
                                    parg);
    case SSL_CTRL_SET_SIGALGS:
        return tls1_set_sigalgs(ctx->cert, parg, larg, 0);
    ...
    case SSL_CTRL_SET_CLIENT_SIGALGS_LIST:
        return GMold_tls1_set_sigalgs_list(ctx->cert, parg, 1);
...
{
    printf("ddddddddddd ssl3_get_cipher_by_char\n");
...
{
    printf("ddddddddddd ssl3_put_cipher_by_char \n");
...
int GMold_ssl3_put_cipher_by_char(const SSL_CIPHER *c, unsigned char *p)
{
    long l;
    ...
    if (p != NULL) {
        ...
        {
            printf("GMold_ssl3_put_cipher_by_char 11111111111111111111 c->id:%d\n",c->id);
            return (0);
        }
        // printf("GMold_ssl3_put_cipher_by_char 222222222222222222 c->id:%d\n",c->id);
...
int GMold_ssl3_shutdown(SSL *s)
{
    ...
    if (!(s->shutdown & SSL_SENT_SHUTDOWN)) {
        s->shutdown |= SSL_SENT_SHUTDOWN;
        GMold_ssl3_send_alert(s, SSL3_AL_WARNING, SSL_AD_CLOSE_NOTIFY);
        /*
         * our shutdown alert has been sent now, and if it still needs to be
         * written, s->s3->alert_dispatch will be true
         */
         ...
         * If we are waiting for a close from our peer, we are closed
         */
        s->method->ssl_read_bytes(s, 0, NULL, NULL, 0, 0,NULL);
        if (!(s->shutdown & SSL_RECEIVED_SHUTDOWN)) {
            return (-1);        /* return WANT_READ */
        }
    }
...
int GMold_ssl3_write(SSL *s, const void *buf, int len)
{
    clear_sys_error();
    if (s->s3->renegotiate)
        GMold_ssl3_renegotiate_check(s,0);

    return s->method->ssl_write_bytes(s, SSL3_RT_APPLICATION_DATA, buf, len,NULL);
}
...
int GMold_ssl3_read(SSL *s, void *buf, int len, size_t *readbytes)
{
...
int GMold_ssl3_renegotiate(SSL *s)
{
    if (s->handshake_func == NULL)
        return (1);
...
{
    int ret = 0;
    printf("ddddddddddd ssl3_renegotiate_check \n");
    if (s->s3->renegotiate) {
...
int GMold_ssl3_renegotiate_check(SSL *s, int initok)
{
    int ret = 0;
    ...
    if (s->s3->renegotiate) {
        if (!RECORD_LAYER_read_pending(&s->rlayer)
             ...
             * state.
             */
            GMold_ossl_statem_set_renegotiate(s);
            s->s3->renegotiate = 0;
            s->s3->num_renegotiations++;
            s->s3->total_renegotiations++;
            ret = 1;
...
int GMold_ssl_fill_hello_random(SSL *s, int server, unsigned char *result, int len)
{
    int send_time = 0;
    ...
    if (send_time) {
        ...
        unsigned char *p = result;
        l2n(Time, p);
        return GMold_RAND_bytes(p, len - 4);
    } else
        return GMold_RAND_bytes(result, len);
}

int GMold_ssl_generate_master_secret(SSL *s, unsigned char *pms, size_t pmslen,
                               int free_pms)
{
